...
import com.hula.manage.FileManager;
import com.hula.manage.upload.FilePictureUpload;
import com.hula.manage.upload.PictureUploadTemplate;
import com.hula.manage.upload.UrlPictureUpload;
import com.hula.mapper.PictureMapper;
import com.hula.model.Picture;
import com.hula.model.Space;
import com.hula.model.User;
import com.hula.model.dto.file.UploadPictureResult;
...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture>
    ...
    public PictureVO uploadPicture(Object inputSource, PictureUploadRequest pictureUploadRequest, User loginUser) {
        ...
        if (spaceId != null) {
            ...
            ThrowUtils.throwIf(space == null, ErrorCode.NOT_FOUND_ERROR, "空间不纯在");
        ...
        if (pictureId != null) {
            ...
            // 校验空间是否一致 因为别人可以通过postman 来传入我这个用户以及其他空间的ID,空间ID是根据用户来获取的
...
//如果以后想要查询公共图库，就把nullSpaceId改成true ,否则就是只给spaceId
        queryWrapper.eq(ObjUtil.isNotEmpty(spaceId), "spaceId", spaceId);
        queryWrapper.isNull(nullSpaceId, "spaceId");// 生成 SQL: WHERE space_id IS NULL
    ...
    public Integer uploadPictureByBatch(PictureUploadByBatchRequest pictureUploadByBatchRequest, User loginUser) {
        ...
        for (Element imgElement : imgElementList) {
            ...
            if (StrUtil.isBlank(namePrefix)) {
                namePrefix = searchText;// 也就是后面你在接口测试的时候，namePrefix 为空，那么就用searchText 来代替
            }
            ...
            if (pictureUploadByBatchRequest.getCategory() != null) {
                pictureUploadRequest.setCategory(pictureUploadByBatchRequest.getCategory());
            }
            if (pictureUploadByBatchRequest.getTags() != null) {
    ...
    public void editPicture(PictureEditRequest pictureEditRequest, User loginUser) {
        ...
        picture.setEditTime(new Date());
        // 数据校验
        this.validPicture(picture);// 判断是否存在
        long id = pictureEditRequest.getId();
        Picture oldPicture = this.getById(id);
        ThrowUtils.throwIf(oldPicture == null, ErrorCode.NOT_FOUND_ERROR);
//        // 仅本人或管理员可编辑(之前是这样的，但是我们刚刚自己封装了一层自己写好的权限校验的Service)
//        if (!oldPicture.getUserId().equals(loginUser.getId()) && !userService.isAdmin(loginUser)) {
//            throw new BusinessException(ErrorCode.NO_AUTH_ERROR);
//        }
        //权限校验
